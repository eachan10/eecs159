...
/*
 * Has some hand optimizations done from main.c. It speeds it up by maybe 10%
 * but at the cost of readability. I don't want to ruin the original main.c
 * because this makes the convolution layer harder to understand possibly
 * for reading when vectorizing. On my laptop it was taking ~3.7ms per inference
 * with just for loops.
*/
      ...
      for (int out_c = 0; out_c < output_w; ++out_c) {
        ...
        for (int in_ch = 0; in_ch < in_channels; ++in_ch) {
          ...
          for (int ker_r = 0; ker_r < kernel_h; ++ker_r) {
            ...
            int in_c = out_c * stride - padding;

            if (in_r >= 0 && in_r < in_h) {  // move this valid row guard out of for loop
              ...
              for (int ker_c = 0; ker_c < kernel_w; ++ker_c) {
                ...
                weight_idx++;
                input_idx++;
                in_c++;  // needed for column bound guard
              }

            } else { weight_idx += kernel_w; }
...
void avgpool2d(int32_t *x, int32_t *out,
  ...
  int scale = kernel_h * kernel_w;
  int output_idx = 0;

  for (int ch = 0; ch < channels; ++ch) {
    for (int out_r = 0; out_r < output_h; ++out_r) {
      for (int out_c = 0; out_c < output_w; ++out_c) {
        int32_t total = rounding;
