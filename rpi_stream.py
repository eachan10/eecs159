...
if __name__ == "__main__":
    ...
    net.load_state_dict(torch.load("out-conv-stop-go-aug/model_iter8.pth", weights_only=True))
    nn.fuse_module(net)
    nn.quantize(net)
    net.eval()
        ...
        while True:
            ...
            if time() - 1 > timestamp:
                ...
                if pred_count >= 1: # PARAM: number of consecutive positives needed to detect for actual positive
                    print(f"\rDETECTED {'STOP' if pred == 1 else 'GO'}                              ", end='')
                    timestamp = time()
                else:
